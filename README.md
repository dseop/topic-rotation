...
## 프로젝트 배경
본 패키지는 개인 레포에서 공용 모듈을 분리해 독립 패키지로 재구성한 결과물입니다. 재사용성을 높이기 위해 API를 정리하고 TypeScript 타입을 강화했으며, `tsc` 빌드·배포 흐름과 기본 테스트를 갖추었습니다. 원본 레포는 비공개이며, 본 저장소는 공개 배포를 위해 필요한 부분만 정리해 제공합니다.
